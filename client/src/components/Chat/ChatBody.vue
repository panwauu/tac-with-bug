<template>
  <div style="height: 100%; display: flex; flex-direction: column;">
    ...
    <div ref="chatBodyMessagesRef" class="chatBodyMessages" @click="messagesStore.markAsRead">
          ...
          <TransitionGroup name="chatmessage">
            ...
            >
              ...
              >
                ...
                >{{ new Date(message.created).toLocaleTimeString(currentLocale, { hour: 'numeric', minute: '2-digit' }) }}</div>
              </div>
            </div>
          </TransitionGroup>
        </div>
    ...
    <div v-if="isLoggedIn" class="chatFooter" @click="messagesStore.markAsRead">
      <form
      ...
      <Message
        ...
        :closable="false"
        style="margin: 0;"
      >{{ $t('Chat.mayNotUseOverlay') }}</Message>
    </div>
    ...
    <OverlayPanel
      ...
      :baseZIndex="1000"
    >
      <ChatGroupEditor v-if="messagesStore.getCurrentChat?.groupChat" @close="closeOverlay" />
      <PrivateChatEditor v-else @close="closeOverlay" />
    </OverlayPanel>
  </div>
</template>
...
function beautifyDate(timestamp: string): string {
  ...
  if (new Date().toDateString() === date.toDateString()) { return i18n.global.t('Chat.Dates.today') }
  else if ((new Date(new Date().getTime() - 1000 * 60 * 60 * 24)).toDateString() === date.toDateString()) { return i18n.global.t('Chat.Dates.yesterday') }
  else if (new Date().getTime() - date.getTime() < 1000 * 60 * 60 * 24 * 7) { return date.toLocaleString(currentLocale.value, { weekday: 'long' }) }
...
function textAreaKeydown(e: KeyboardEvent) {
  if (e.key === 'Enter' && !e.shiftKey) {
    e.preventDefault();
    if (inputMessage.value.length <= 500) { submitChatInput() }
  }
}
...
function scrollDown() { chatBodyMessagesRef.value?.scroll(0, 0) }
</script>
