    ...
    >
      <i
        :class="[
          ...
          'pi-chevron-down',
          'menuChevron',
          { menuChevronOpen: displayMenu },
        ]"
      />
    </Button>
    <Menu ref="menuRef" :model="userMenu" :popup="true" :baseZIndex="2000" />
  </div>
</template>
...
onMounted(() => {
    watch(
        ...
        () => { setTimeout(() => { userMenu.value = returnMenu() }, 200) }
    )

    watch(
        () => { return (menuRef.value as any)?.overlayVisible },
        (val: boolean) => { displayMenu.value = val }
    );
