...
const userAgentChartOptions = {
    ...
    plugins: {
        tooltip: {
            callbacks: {
                label: function (context: any) { return `${context.label}: ${(context.parsed * 100).toFixed(2)} %` }
            }
        }
    }
}
...
function groupAndSortData(data: Record<string, number>, threshold: number, groupKeyName: string): [string, number][] {
    ...
    entries.forEach((e) => {
        if (e[1] / overallDataPoints < threshold) { othersElement[1] += e[1] }
        else { resultingArray.push(e) }
    })
