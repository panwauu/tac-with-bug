<template>
  <div style="display: flex; overflow-x: auto; min-width: 20px; min-height: 20px;">
    <div v-if="emojiListEditing" class="p-inputgroup">
      <InputText v-model="newEmoji" style="width: 40px;" />
      <Button
        icon="pi pi-plus"
...
function sendEmoji(emoji: string) {
    if (emojiListEditing.value) { return }
...
const newEmoji = ref('')
const validNewEmoji = computed(() => { return isEmoji(newEmoji.value) && !emojiList.value.includes(newEmoji.value) })
function addEmoji() {
    if (validNewEmoji.value) { emojiList.value = [newEmoji.value].concat(emojiList.value) }
    newEmoji.value = ''
}
