<template>
  <span class="p-float-label">
    ...
    <small
      ...
      class="p-error"
    >{{ $t("Tournament.SignUp.invalidTeamName") }}</small>
    <small
      v-if="!(localTeamName === '' || newName) && !disabled"
      class="p-error"
    >{{ $t("Tournament.SignUp.notNewTeamName") }}</small>
  </span>
</template>
...
const validTeamName = computed(() => {
  return localTeamName.value.trim() === localTeamName.value &&
...
const newName = computed(() => { return !props.existingTeamNames.some((t) => t === localTeamName.value) })

const validName = computed(() => { return (newName.value && validTeamName.value) || props.disabled })

watch(() => [validName.value], () => { emit('update:valid', validTeamName.value) }, { deep: true })
</script>
