  ...
  <div class="p-card subscriptionPage">
    ...
    <div v-show="isLoggedIn">
      <Message
        ...
        :closable="false"
        severity="success"
      >{{ $t("Subscription.freelicense") }}</Message>
      ...
      <div v-if="subscriptionState.status === 'running'">
        ...
        <CountdownTimer :endDate="subscriptionState.validuntil ?? undefined" />
        <Button :label="$t('Subscription.Cancel.button')" @click="cancelSubscription()" />
      </div>
      <div
        v-show="
      ...
      >
        ...
        <SelectButton v-model="selectedPlan" :options="planModel" optionLabel="name" />
        <Message
          :closable="false"
          :severity="paypalPercentageSeverity"
          class="paypalPercentage"
        >{{ paypalPercentage + $t("Subscription.paypalFeeDisclaimer") }}</Message>
    ...
    <div style="margin-top: 10px">
      ...
      <div class="disclaimer">
        ...
        >{{ $t("Subscription.disclaimer3Link") }}</a>
      </div>
    </div>
  </div>
</template>
...
function saveNSubscriptions(data: number) { nSubscriptions.value = data }
...
function renderButton() {
  if (button.value != null) { button.value?.close() }

  paypal.value.then((paypal) => {
    if (paypal === null) {
