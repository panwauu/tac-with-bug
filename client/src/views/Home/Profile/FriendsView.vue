<template>
  <div>
    <DataTable :value="friendsForTable">
      <Column field="username" :header="$t('Profile.Friends.username')">
        <template #body="slotProps">
          <PlayerWithPicture :username="slotProps.data.username" :nameFirst="false" />
        </template>
      </Column>
      <Column field="date" :header="$t('Profile.Friends.friendssince')">
        <template #body="slotProps">{{ new Date(slotProps.data.date).toLocaleDateString() }}</template>
      </Column>
      <Column v-if="username === loggedInUsername" field="status">
        <template #body="slotProps">
  ...
  try {
    ...
    friends.value = await new Promise<friend[]>((resolve, reject) => {
      ...
      socket.emit('friends:ofUser', props.username, (res) => {
        if (res.data === undefined) { return reject(new Error('Server Side Error ')) }
        resolve(res.data)
      });
    })
    loading.value = false
...
const friendsForTable = computed(() => {
  if (props.username === loggedInUsername.value) { return friendsState.friends }
