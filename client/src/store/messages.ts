...
export const useMessagesStore = defineStore('messages', {
    state: () => ({
        ...
        channels: [{ id: 'general', missedMessages: 0, endDate: null as null | number }, { id: 'news', missedMessages: 0, endDate: null }],
    ...
    actions: {
        async initStore() {
            ...
            this.$state.socket.on('chat:overview:update', (chats) => { this.handleOverviewUpdate(chats) })

            this.loadChatMessages()
            this.$state.socket.on('chat:singleChat:update', (data) => { this.handleSingleChatUpdate(data.chatid, data.messages) })
            this.$state.socket.on('channel:update', (data) => { this.handleChannelUpdate(data.channel, data.messages, true) })
        },
        loadChatMessages() {
            this.chatMessages = []
        ...
        handleSingleChatUpdate(chatid: number, messages: chatMessage[]) {
            if (this.selectedChat.type === 'chat' && chatid === parseInt(this.selectedChat.id)) { this.chatMessages = messages }
        },
        ...
        expandChatGroup(index: number) {
            if (index >= this.expandedChats.length) { return }
        ...
        selectChat(channel: boolean, id: string) {
            ...
            const chatStore = useChatStore()
            if (chatStore.responsiveMode) { chatStore.closeChatMenu() }
        },
        async markAsRead() {
            if (this.selectedChat.type === 'chat') { await this.$state.socket.emitWithAck(2000, 'chat:markAsRead', { chatid: parseInt(this.selectedChat.id) }) }
            else {
                const channel = this.channels.find((c) => c.id === this.selectedChat.id)
                if (channel != null) { channel.missedMessages = 0 }
            }
        },
        ...
        async startChat(users: { id: number, username: string }[], title: string | null) {
            if (title == null) {
                const chat = this.chats.find((c) => { return !c.groupChat && c.players.includes(users[0].username) })
                if (chat != null) {
        ...
        updateGameChannels(newRunningGameIDs: number[], removedRunningGameIDs: number[], newRouteGameID: string | null, oldRouteGameID: string | null) {
            removedRunningGameIDs.forEach((id) => {
                ...
                if (channel != null) { channel.endDate = new Date().getTime() + keepGameChannelAliveTime }
...
watch(() => router.currentRoute.value.query.gameID, (newGameID, oldGameID) => {
    const messagesStore = useMessagesStore()
...
nextTick(() => {
    ...
    watch(() => serverInfoStore.runningGames, (newGames, oldGames) => {
        ...
        messagesStore.updateGameChannels(newGamesOfPlayer.map((g) => g.id), removedGamesOfPlayer.map((g) => g.id), null, null)
    }, { deep: true })
})

export function formatChannelName(channelName: string, isChannel: boolean) {
    if (isChannel && channelName.substring(0, 2) === 'g-') { return `${i18n.global.t('Chat.Channels.Game')} ${channelName.substring(2)}` }
    if (isChannel && channelName.substring(0, 2) === 'w-') { return `${i18n.global.t('Chat.Channels.Waiting')} ${channelName.substring(2)}` }
    if (isChannel) { return i18n.global.t(`Chat.Channels.${channelName}`) }
    return channelName
}
...
function channelSortFunction(a: { id: string }, b: { id: string }) {
    ...
    if (a.id === 'news') { return -1 }
    if (b.id === 'news') { return 1 }
