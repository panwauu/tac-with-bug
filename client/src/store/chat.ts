...
export const useChatStore = defineStore('chat', {
  ...
  actions: {
    ...
    switchPosition(position: 'right' | 'bottom' | 'overlay') {
    ...
    updateButtonCollapse() {
      ...
      this.collapseButton = element != null && window.innerWidth > window.innerHeight && window.innerWidth - element.getBoundingClientRect().right < 76
    },
    toggleChatMenu() {
      this.displayChatMenu = !this.displayChatMenu
    },
    closeChatMenu() {
      this.displayChatMenu = false
    },
    setResponsiveMode(responsive: boolean) {
      this.responsiveMode = responsive
    },
  },
})
...
watch(
  ...
  () => {
    ...
    setTimeout(() => {
      chatStore.updateButtonCollapse()
    }, 1000)
  }
)
