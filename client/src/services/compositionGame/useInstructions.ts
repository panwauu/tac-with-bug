...
export interface instructionsStateType { instructions: string[] }
...
export function useInstructions(miscState: miscStateType, ballsState: ballsStateType, cardsState: cardsStateType): instructionsStateType {
    ...
    watch([miscState, ballsState, cardsState], () => {
        if (!miscState.players || !(typeof miscState.players !== 'undefined' && miscState.players.length > 0) || miscState.gamePlayer === -1) { return }
