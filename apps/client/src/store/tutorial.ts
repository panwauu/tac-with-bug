...
export const useTutorialStore = defineStore('tutorial', {
  ...
  getters: {
    getProgress(state): boolean[][] {
      return state.progress
    },
  },
  actions: {
    initStore() {
      this.$state.socket.on('tutorial:loadProgress', (progress) => {
...
function validateProgress(progressToValidate: any, referenceProgress: boolean[][]): void {
  if (!Array.isArray(progressToValidate)) {
    throw new Error('Progress could not be validated')
  }
  if (progressToValidate.length !== referenceProgress.length) {
    throw new Error('Progress could not be validated')
  }
  for (let i = 0; i < referenceProgress.length; i++) {
    if (!Array.isArray(progressToValidate[i])) {
      throw new Error('Progress could not be validated')
    }
    if (progressToValidate[i].length !== referenceProgress[i].length) {
      throw new Error('Progress could not be validated')
    }
    if (progressToValidate[i].some((e: any) => typeof e !== 'boolean')) {
      throw new Error('Progress could not be validated')
    }
  }
}
