...
type Head<T extends any[]> = T extends [...infer Head, any] ? Head : any[] // eslint-disable-line no-shadow
type Last<T extends any[]> = T extends [...any, infer Last] ? Last : any // eslint-disable-line no-shadow
...
type EventsWithCallback<T> = {
  [P in keyof T]: T[P] extends (...args: infer A) => void // Check if it is a function
    ? Last<A> extends (arg: any) => any // Check if the last parameter is a function with single parameter
      ? ReturnType<Last<A>> extends void // eslint-disable-line @typescript-eslint/no-invalid-void-type
