...
async function validateTutorialIDAndStep(tutorialID: number, tutorialStep: number): Promise<Result<null, ValidateTutorialIDAndStepError>> {
  ...
  if (!Number.isInteger(tutorialStep) || tutorialStep < 0 || tutorialStep >= tutorialLevels[tutorialID]) {
    return err('TUTORIAL_STEP_NOT_VALID')
  }
  return ok(null)
}

export async function getDefaultTutorialProgress(pgPool: pg.Pool): Promise<boolean[][]> {
  const dbRes = await pgPool.query<{ tutorial: string }>(
    "SELECT to_jsonb(column_default) as tutorial FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'users' AND column_name = 'tutorial';"
  )
  if (dbRes.rowCount !== 1) {
    throw new Error('Tutorial progress default value could not be queried')
  }
...
export async function resetTutorialProgress(pgPool: pg.Pool, userID: number, tutorialID: number) {
  ...
  const newProgressRow = JSON.stringify(new Array(tutorialLevels[tutorialID]).fill(false))
  const dbRes = await pgPool.query<{ tutorial: boolean[][] }>('UPDATE users SET tutorial = jsonb_set(tutorial, ARRAY[$2::text], $3) WHERE id = $1 RETURNING *;', [
    userID,
    tutorialID,
    newProgressRow,
  ])
...
export async function setTutorialProgress(
  ...
  const dbRes = await pgPool.query<{ tutorial: boolean[][] }>('UPDATE users SET tutorial = jsonb_set(tutorial, ARRAY[$2::text,$3::text], $4) WHERE id = $1 RETURNING *;', [
    userID,
    tutorialID,
    tutorialStep,
    done,
