...
function expandNode(node: EndNode): EndNode[] {
  ...
  const duplicatedCardIndices = new Set(
    node.state.cardsWithMoves.map((_, i) => i).filter((i) => node.state.cardsWithMoves.map((c) => c.title).indexOf(node.state.cardsWithMoves[i].title) !== i)
  )
