...
export class Game implements GameData {
  ...
  constructor(nPlayers: number, nTeams: number, meisterVersion: boolean, coop: boolean, gameLoad?: GameData, logger: Logger = noopLogger) {
    ...
    if (gameLoad == null) {
      ...
      this.winningTeams = Array.from({ length: this.teams.length }, () => false)
      this.cardsWithMoves = []
      ...
      this.tradedCards =
        ...
        ((gameLoad as any).tradeCards == null
          ...
          : (gameLoad as any).tradeCards.map((c: string) => (c === '' ? null : c)))
  ...
  checkWinningTeams(): boolean[] {
    const winningArray: boolean[] = Array.from({ length: this.teams.length }, () => false)
  ...
  updateCardsWithMoves(): void {
    ...
    if (this.gameEnded) {
      for (const card of this.cards.players[activePlayer]) {
      ...
      for (const card of this.cards.players[activePlayer]) {
        let cardWithMove = createCardWithMove(card, this.balls, -1, activePlayer, this.teams, this.cards, this.coop, this.sevenChosenPlayer)

        if (card === 'tac') {
          cardWithMove = createCardWithMove('8', this.priorBalls, activePlayer, activePlayer, this.teams, this.cards, this.coop, this.sevenChosenPlayer)
          cardWithMove.title = 'tac'
        }
        cardWithMove.possible = true
        cardWithMove.textAction = cardWithMove.textAction + '+abwerfen'
        cardsWithMoves.push(cardWithMove)
      }
    } else {
      for (const card of this.cards.players[activePlayer]) {
        if (this.tradeFlag === true) {
          ...
          const tacCard: tCard.PlayerCard =
            lastNonTacCard === undefined
              ? createCardWithMove('1', this.balls, activePlayer, -1, this.teams, this.cards, this.coop, this.sevenChosenPlayer)
              : createCardWithMove(lastNonTacCard, this.priorBalls, activePlayer, activePlayer, this.teams, this.cards, this.coop, this.sevenChosenPlayer)
          tacCard.title = 'tac'
          cardsWithMoves.push(tacCard)
        } else if (card.startsWith('tac-')) {
          const tacCard = createCardWithMove('7' + card.slice(3), this.balls, activePlayer, activePlayer, this.teams, this.cards, this.coop, this.sevenChosenPlayer)
          tacCard.title = 'tac' + tacCard.title.slice(1)
          cardsWithMoves.push(tacCard)
        } else {
    ...
    if (cardsWithMoves.some((card) => card.title.startsWith('7-') || card.title.startsWith('tac-'))) {
      for (const card of cardsWithMoves.filter((card) => !(card.title.startsWith('7-') || card.title.startsWith('tac-')))) {
        card.possible = false
        card.ballActions = {}
        card.textAction = ''
      }
    } else if (this.winningTeams.includes(true)) {
  ...
  performActionAfterStatistics(move: tBall.MoveTextOrBall): void {
    ...
    } else if (this.narrFlag.includes(true) && move[2] === 'narr') {
  ...
  determineGameEnded() {
    if (this.coop === false) {
      ...
      if (
        ...
        this.gameEnded = true
      } else if (winningArray.includes(true) && this.cards.players[this.activePlayer].length === 0) {
