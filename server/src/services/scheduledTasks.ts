...
export async function registerJobs(sqlClient: pg.Pool) {
  if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'production') {
    jobs.push(
      schedule.scheduleJob({ rule: '0 * * * * *', tz: 'Europe/Berlin' }, () => {
        removeDoneJobs()
      })
    )
    jobs.push(
      schedule.scheduleJob({ rule: '0 0 0 * * *', tz: 'Europe/Berlin' }, async () => {
        await DeleteUnactivatedUsers(sqlClient)
      })
    )
    jobs.push(
      schedule.scheduleJob({ rule: '0 * * * * *', tz: 'Europe/Berlin' }, async () => {
        await abortNotEndedGames(sqlClient)
        await endNotProperlyEndedGames(sqlClient)
      })
    )
    jobs.push(
      schedule.scheduleJob({ rule: '*/5 * * * * *', tz: 'Europe/Berlin' }, async () => {
...
export function registerJobsForOneTournament(sqlClient: pg.Pool, tournament: publicTournament) {
  ...
  jobs.push(
    schedule.scheduleJob(new Date(new Date(tournament.signupBegin).getTime() + tournamentTimeOffset), async () => {
      await startSignUpOnCondition(sqlClient)
    })
  )

  jobs.push(
    schedule.scheduleJob(new Date(new Date(tournament.signupDeadline).getTime() + tournamentTimeOffset), async () => {
      await endSignUpOnCondition(sqlClient)
    })
  )
  ...
  tournament.creationDates.forEach((date, i) => {
    ...
    jobs.push(
      schedule.scheduleJob(new Date(new Date(date).getTime() + timePerGameInMS + tournamentTimeOffset), async () => {
        await checkForceGameEnd(sqlClient)
      })
    )
  })
