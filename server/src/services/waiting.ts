...
export async function getWaitingGames(sqlClient: pg.Pool, waitingGameID?: number) {
    const res = await sqlClient.query(`SELECT waitinggames.*, 
    ...
    ${waitingGameID != null ? 'WHERE waitinggames.id = $1' : ''};`, (waitingGameID != null ? [waitingGameID] : []))
...
export async function createRematchGame(pgPool: pg.Pool, game: gameForPlay, userID: number): Promise<Result<number, createRematchError>> {
    if (!game.rematch_open) { return err('REMATCH_NOT_OPEN') }
    const waitingGames = await getWaitingGames(pgPool)
    await disableRematchOfGame(pgPool, game.id)
    game.rematch_open = false
    if (waitingGames.some((g) => g.players.some((waitingPlayer: string) => game.players.includes(waitingPlayer)))) { return err('PLAYER_ALREADY_IN_WAITING_GAME') }
    if (game.playerIDs.some((id) => !isUserOnline(id))) { return err('PLAYER_NOT_ONLINE') }

    let nTeams = game.game.teams.length;
    if (game.game.coop) { nTeams = 1 }
    ...
    const query = `INSERT INTO waitinggames (private, adminplayer, nPlayers, nTeams, meister, gameid ${ballsStr} ${playersStr}) VALUES (true, $1, $2, $3, $4, $5 ${valStr} ${selectStr}) RETURNING id;`
    const createRes = await pgPool.query<{ id: number }>(query, values)
    return ok(createRes.rows[0].id)
}

export type movePlayerError = 'PLAYER_NOT_FOUND_IN_WAITING_GAME' | 'PLAYER_NOT_ALLOWED_TO_MOVE' | 'PLAYER_CANNOT_BE_MOVED_IN_DIRECTION' | getWaitingGameError | notOneDatabaseChangeError
export async function movePlayer(sqlClient: pg.Pool, waitingGameID: number, usernameToMove: string, up: boolean, userID: number): Promise<Result<null, movePlayerError>> {
    ...
    let readyToFalse = '';
    [0, 1, 2, 3, 4, 5].forEach((n) => { readyToFalse += `, ready${n}=false` });

    const query = `UPDATE waitinggames SET (${playerFirst}, ${playerSecond}, ${ballsFirst}, ${ballsSecond}) = (${playerSecond}, ${playerFirst}, ${ballsSecond}, ${ballsFirst}) ${readyToFalse} WHERE id=$1;`
    const values = [waitingGameID]
    const dbRes = await sqlClient.query(query, values)
    if (expectOneChangeToDatabase(dbRes).isErr()) { return err('NONE_OR_MORE_THAN_ONE_CHANGES_TO_DATABASE') }
    return ok(null)
}

export type changeColorError = 'PLAYER_NOT_FOUND_IN_WAITING_GAME' | 'PLAYER_NOT_ALLOWED_TO_CHANGE_COLOR' | 'COLOR_DOES_NOT_EXIST' | 'COLOR_ALREADY_IN_USE' | getWaitingGameError | notOneDatabaseChangeError
    ...
    for (const waitingGame of waitingGames) {
        ...
        if (waitingGame.gameid != null || waitingGame.playerIDs.filter((id) => id != null).length <= 1) {
            ...
            p.push(sqlClient.query(`UPDATE waitinggames SET 
                ...
                ready0=false, ready1=false, ready2=false, ready3=false, ready4=false, ready5=false WHERE id=$1;`, [waitingGame.id]))
        }
    }
...
export async function addPlayer(sqlClient: pg.Pool, waitingGameID: number, userID: number): Promise<Result<null, addPlayerError>> {
    ...
    if (insertIndex === -1) { return err('WAITING_GAME_IS_ALREADY_FULL') }

    const color = colors.find((c) => !game.value.balls.includes(c))
    if (color === undefined) { return err('COULD_NOT_FIND_COLOR_FOR_NEW_PLAYER') }
...
export async function getPlayersOfWaitingGame(sqlClient: pg.Pool, waitingGameID: number): Promise<number[]> {
    return sqlClient.query('SELECT player0, player1, player2, player3, player4, player5 FROM waitinggames WHERE player0 = $1 OR player1 = $1 OR player2 = $1 OR player3 = $1 OR player4 = $1 OR player5 = $1;', [waitingGameID])
        .then((res) => { return res.rowCount === 0 ? [] : Object.values(res.rows[0]) })
}