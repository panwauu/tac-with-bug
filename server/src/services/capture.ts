...
export async function insertCompleteCapture(sqlClient: pg.Pool, gameID: number, game: string) {
    ...
    if (array.length <= 0) { throw new Error('game is empty') }
    if (['balls', 'cards', 'action', 'activePlayer'].some((k) => array.some((r: any) => !Object.keys(r).includes(k)))) { throw new Error('game misses keys') }
