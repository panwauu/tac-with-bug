...
export async function createTournament(sqlClient: pg.Pool, title: string, begin: string, deadline: string, creationDates: string[], secondsPerGame: number, nTeams: number, playersPerTeam: 2 | 3, teamsPerMatch: 2 | 3, tournamentType: 'KO'): Promise<Result<tTournament.publicTournament, createTournamentError>> {
    ...
    const dbRes = await sqlClient.query(`INSERT INTO tournaments (title, status, signup_begin, signup_deadline, creation_dates, time_per_game, n_teams, players_per_team, tournament_type, data, teams_per_match)
