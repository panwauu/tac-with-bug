...
import type { KoBracket, PrivateTournament, PublicTournament, TournamentDataKO } from '../sharedTypes/typesTournament.js'
import { shuffleArray } from '../game/cardUtils.js'
...
export function createTournamentDataKO(nTeams: number, teamsPerMatch: 2 | 3): Result<TournamentDataKO, CreateTournamentDataKOError> {
  ...
  if (nRounds > 1) {
    // Create Bracket for 3rd place game
  ...
  for (const [bracketIndex, b] of brackets[0].entries()) {
    for (const [teamIndex] of b.teams.entries()) {
      ...
      if (next === undefined) {
        return err('RANDOM_TEAM_CREATION_ERROR')
      }
...
function findBracket(tournament: PublicTournament | PrivateTournament, gameID: number): Result<[number, number], FindBracketError> {
  ...
  if (firstIndex === -1) {
    return err('GAME_NOT_IN_KO_BRACKETS')
  }
...
function getWinnerOfTournamentGame(game: GameForPlay, bracket: KoBracket, tournament: PublicTournament | PrivateTournament): Result<number, GetWinnerOfTournamentGameError> {
  if (!game.running && game.game.gameEnded) {
    ...
    if (bracketWinningTeam === undefined) {
      return err('WINNER_OF_TOURNAMENT_GAME_NOT_FOUND')
    }
  ...
  if (
    game.game.statistic.every((pS) => {
      return pS.actions.nMoves === 0
    })
  ) {
...
export function evaluateGameWinnerAndReturnEndedFlag(
  ...
  if (pos.value[0] === tournament.data.brackets.length - 2) {
    ...
    if (looser === undefined) {
      return err('LOOSER_COULD_NOT_BE_FOUND')
    }
