...
export class GameController extends Controller {
    ...
    public async searchPlayers(
        ...
        return dbRes.rows.map((entry: any) => { return { username: entry.username, id: entry.id } })
    }
    ...
    @Security('jwt')
    @Delete('/abortGame')
    public async abortGame(
        @Request() request: express.Request,
        @Body() requestBody: { gameID: number },
        @Res() authorizationError: TsoaResponse<403, any>,
    ): Promise<void> {
