...
export class ActivationController extends Controller {
  ...
  public async activateUser(
    ...
    if (user.isErr()) {
      return userNotFoundResponse(404, 'Validation failed: userID not found')
    }

    if (user.value.token !== token) {
      return tokenWrongResponse(403, 'Validation failed: wrong token')
    }
  ...
  public async requestNewActivationMail(
    ...
    if (user.isErr()) {
      return usernameNotFoundResponse(404, 'Username not found')
    }

    if (user.value.activated === true) {
      return alreadyActivatedError(409, 'User is already activated')
    }
