...
export class TournamentsController extends Controller {
    ...
    public async generateTournamentTeamName(
        ...
        if (result.isErr()) { return serverError(500, { message: result.error }) }
        return result.value
    }
    ...
    public async createTournament(
        ...
        const tournament = await createTournament(request.app.locals.sqlClient, requestBody.title, requestBody.begin, requestBody.deadline, requestBody.creationDates, requestBody.secondsPerGame, requestBody.nTeams, 2, 2, 'KO')
        if (tournament.isErr()) { return serverError(500, { message: 'Internal Server Error', details: tournament.error }) }
    ...
    public async exchangeTournamentUser(
        ...
        const idRes = await request.app.locals.sqlClient.query('SELECT id, username FROM users WHERE username = $1 OR username = $2;', [requestBody.usernameToReplace, requestBody.usernameOfReplacement])
