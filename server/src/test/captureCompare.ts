...
export function repeatGame(lines: any[], nPlayersParam?: number, nTeamsParam?: number, meisterVersionParam?: boolean, coopParam?: boolean) {
    ...
    if (!compareGameWithCaptured(gameInst, lineInit)) { console.log('Sanity-Check: failed'); return result }
...
function compareGameWithCaptured(gameInst: game, capturedState: any) {
    ...
    if (compareCards(gameInst.cards, capturedState.cards) === false) {
        ...
        Object.keys(gameInst.cards).forEach((k) => {
            if (typeof (gameInst.cards as any)[k] === 'object') { dataForTable[k] = [JSON.stringify((gameInst.cards as any)[k]), JSON.stringify(capturedState.cards[k])] }
            else { dataForTable[k] = [(gameInst.cards as any)[k], capturedState.cards[k]] }
        })
...
function compareBalls(balls1: any, balls2: any) {
    ...
    for (let i = 0; i < balls1.length; i++) {
        if (balls1[i].state !== balls2[i].state &&
            !(balls1[i].state === 'goal' && (balls2[i].state === 'goalForward' || balls2[i].state === 'goalBackward'))) {
