...
export async function getToken(): Promise<string> {
  ...
  if (token === null) {
    throw new Error('Cannot return null as a token')
  }
  return token
}
...
export async function requestTokenFromPaypal(): Promise<void> {
  ...
  try {
    ...
    token = new Promise((resolve) => {
      ...
      axios.post(`https://api-m.${process.env.NODE_ENV === 'production' ? '' : 'sandbox.'}paypal.com/v1/oauth2/token?grant_type=client_credentials`, {}, config).then((res) => {
