...
export function performBallAction(card: tCard.playerCard, nBall: number, newPosition: number, cardIndex: number, balls: tBall.ballsType, activePlayer: number, cards: tCard.cardsType, priorBalls: tBall.ballsType, teufelflag: boolean): void {
...
function updatePriorBalls(card: tCard.playerCard, balls: tBall.ballsType, priorBalls: tBall.ballsType): void {
    if (card.title === 'tac') { // If tac is player -> load priorballs
        const auxBalls = cloneDeep(balls)
        Object.assign(balls, cloneDeep(priorBalls))
        Object.assign(priorBalls, cloneDeep(auxBalls))
    }
    else if (card.title.substring(0, 3) === 'tac' && card.title.length > 3) { return } // If 7 was taced and is continued
    else if (card.title.substring(0, 2) === '7-') { return } // If 7 was played and is continued
    else { // Copy balls into priorBalls
...
function performTrickser(balls: tBall.ballsType, nBall: number, newPosition: number): void {
    const nBallInDestination = balls.findIndex(ball => { return ball.position === newPosition })
...
function kickBalls(balls: tBall.ballsType, card: tCard.playerCard, newPosition: number, nBall: number, lastNonTacCard: tCard.cardType | undefined): void {
    const nBallInDestination = balls.findIndex(ball => { return ball.position === newPosition })
    if (nBallInDestination !== -1) { // move ball in goal position back to house
        moveBallToHouse(balls, nBallInDestination)
    }

    if (card.title[0] === '7' || (card.title.substring(0, 3) === 'tac' && lastNonTacCard === '7')) { // kick all balls inbetween if "7"
...
export function moveBallToHouse(balls: tBall.ballsType, nBall: number): void {
    ...
    if (emptyHousePosition === -1) { throw new Error('Could not find House Position') }
...
function updateState(balls: tBall.ballsType, nBall: number, newPosition: number, remainingMoves: number): void {
    if (balls[nBall].state === 'house') { // just left house -> not allowed to enter goal area
        balls[nBall].state = 'invalid'
    }
    else if (balls[nBall].state === 'invalid') { // just left start position -> allowed to enter goal area
        balls[nBall].state = 'valid'
    }
    else if (newPosition >= ballGoal(nBall, balls)) { // ball is in goal
        if (remainingMoves === 0) { // move ended
            if (ballInLastGoalPosition(balls, nBall, newPosition)) { // is in last position -> locked
                balls[nBall].state = 'locked'
            }
            else { // is not in last position -> goal
                balls[nBall].state = 'goal'
            }
        }
        else { // move not ended - definitively seven
...
function getRemainingMoves(card: tCard.playerCard, balls: tBall.ballsType, nBall: number, newPosition: number, lastNonTacCard: tCard.cardType | undefined): number {
    ...
    if (card.title[0] === '7' || (card.title.substring(0, 3) === 'tac' && lastNonTacCard === '7')) { // reset all balls inbetween if "7"
        let priorRemainingMoves = 7
        if (card.title.substring(0, 2) === '7-') { priorRemainingMoves = parseInt(card.title.substring(2, card.title.length)) }
        if (card.title.substring(0, 3) === 'tac' && card.title.length > 3) { priorRemainingMoves = parseInt(card.title.substring(4, card.title.length)) }
        remainingMoves = priorRemainingMoves - (sevenReconstructPath(balls, nBall, newPosition).length - 1)
    }
    return remainingMoves
}

function updateCardAfter7(card: tCard.playerCard, cards: tCard.cardsType, teufelflag: boolean, activePlayerParam: number, remainingMoves: number, cardIndex: number, lastNonTacCard: tCard.cardType | undefined) {
...
export function sevenReconstructPath(balls: tBall.ballsType, nBall: number, goalPosition: number): number[] {
    ...
    if (finalPath === undefined) { throw new Error('Could not reconstruct Path of 7') }
    return finalPath
}
