...
export class game {
    ...
    getJSON(): string { return JSON.stringify(this) }
    ...
    updateCardsWithMoves(): void {
        ...
        if (this.gameEnded) {
            ...
            this.cards.players[activePlayer].forEach(card => {
                if (this.tradeFlag === true) {
                    ...
                    const tacCard = createCardWithMove('7' + card.substring(3, card.length), this.balls, activePlayer, activePlayer, this.teams, this.cards, this.coop, this.sevenChosenPlayer)
        ...
        if (cardsWithMoves.some(card => (card.title.substring(0, 2) === '7-') || (card.title.substring(0, 4) === 'tac-'))) {
            cardsWithMoves.filter(card => !((card.title.substring(0, 2) === '7-') || (card.title.substring(0, 4) === 'tac-'))).forEach((card) => {
                card.possible = false
                card.ballActions = {}
                card.textAction = ''
            })
    ...
    checkMove(move: tBall.moveType): boolean {
        if (move === 'dealCards') { return this.cards.players.every((p) => p.length === 0) }

        if (this.tradeFlag) { return this.tradeCards[move[0]] === '' }
        ...
        if (!card.possible) { logger.info('card not possible'); return false }

        if (move.length === 3) {
            if (!card.textAction.includes(move[2])) { logger.info('textAction not allowed'); return false }
        }
    ...
    performActionAfterStatistics(move: tBall.moveTextOrBall): void { // Change move representation -> [PlayerNumber, CardNumber, (TextAction / BallNumber, newBallPosition)]
        if (move[2] === 'tauschen') {
            ...
            if (move.length === 3) {
                ...
                if (this.aussetzenFlag) { this.aussetzenFlag = false }
    ...
    determineGameEnded() {
        if (this.coop === false) {
            ...
            if (winningArray.some((element, teamIndex) => { return (element === true && this.winningTeams[teamIndex] === true) })) {
                this.gameEnded = true
    ...
    performTextAction(card: tCard.playerCard, move: tBall.moveText): void {
        ...
        if (cardTextAction === 'beenden') { return }

        discardCard(this.cards, move[1], this.activePlayer, this.teufelFlag)
        if (this.teufelFlag === true) { this.nextPlayer() }
        if (this.teufelFlag === true && cardTextAction === 'narr') { this.priorPlayer() }
        ...
        switch (cardTextAction) {
            case 'abwerfen':
                if (this.aussetzenFlag === true) { this.aussetzenFlag = false }
                break;
            case 'narr':
                this.cards.players.forEach((cards, index) => {
                    if (cards.length === 0) { this.narrFlag[index] = true }
                })
                this.performNarrAction(move)
                break;
            case 'teufel':
    ...
    checkCards() { checkCardsAndDeal(this) }
}
