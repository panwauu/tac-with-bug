...
export function narrCardSwap(cards: tCard.cardsType): void {
    ...
    if (firstPlayerCards === undefined) { throw new Error('Card Swap Failed as cards.players is empty') }
...
export function addCardToDiscardPile(cards: tCard.cardsType, cardIndex: number, activePlayer: number, teufelFlag: boolean) {
    ...
    if (cards.discardPile[cards.discardPile.length - 1][0] === '7') { cards.discardPile[cards.discardPile.length - 1] = '7' }
    if (cards.discardPile[cards.discardPile.length - 1].substring(0, 3) === 'tac') { cards.discardPile[cards.discardPile.length - 1] = 'tac' }
}
...
export function createCardDeck(nPlayers: number, meisterVersion: boolean): Array<tCard.cardType> {
    ...
    for (const [key, value] of Object.entries(cardCount)) {
        for (let i = 0; i < value; i++) { deck.push(key) }
    }
