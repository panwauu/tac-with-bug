...
export function createCardWithMove(cardTitle: tCard.cardType, balls: tBall.ballsType, player: number, activePlayer: number, teams: number[][], cards: tCard.cardsType, coop: boolean, sevenChosenPlayer: number | null) {
    const card: tCard.playerCard = { 'title': cardTitle, 'possible': false, 'ballActions': {}, 'textAction': '' }
    if (player !== activePlayer) { return card }

    if (cardTitle === 'tac') { throw new Error('cannot createCardWithMove with tac Card'); }

    let playablePlayers = getPlayablePlayers(balls, player, teams, coop, sevenChosenPlayer)
    if (cardTitle === 'engel') { playablePlayers = getPlayablePlayers(balls, (player + 1) % (balls.length / 4), teams, coop, sevenChosenPlayer) }

    for (let nBall = 0; nBall < balls.length; nBall++) {
        if (cardTitle === 'trickser') {
            if (!(balls.filter(ball => playablePlayers.includes(ball.player)).some(ball => ball.position >= ballStart(0, balls) && ball.position < ballGoal(0, balls)))) { continue; }
        }
        else if (playablePlayers.includes(balls[nBall].player) === false) { continue; }
    ...
    if (card.title === 'engel' && card.possible === false) {
        for (let nBall = 0; nBall < balls.length; nBall++) {
            if (playablePlayers.includes(balls[nBall].player) === false) { continue; }
    ...
    else if (card.title === '8' && (balls.some(ball => ((playablePlayers.includes(ball.player)) && (ball.position >= ballStart(0, balls)) && (ball.position < ballGoal(0, balls)))
...
export function getMoves(balls: tBall.ballsType, nBall: number, cardTitle: string, teams: number[][], coop: boolean): number[] {
    ...
    if (cardTitle === 'krieger') {
        ...
        if (cardTitle.length > 1) { remainingMoves = parseInt(cardTitle.substring(2, cardTitle.length)) }
        movePositions = getSevenPositions(balls, nBall, remainingMoves, teams, coop)
    }
    else { // Cards that can be moved ordinarily
...
export function getKriegerMove(balls: tBall.ballsType, nBall: number): number[] {
    if (balls[nBall].position >= ballGoal(0, balls) || balls[nBall].position < ballStart(0, balls)) { return [] }

    const largerPos = []
    const smallerPos = []
    for (let i = 0; i < balls.length; i++) {
        if (balls[i].position >= ballStart(0, balls) && balls[i].position < ballGoal(0, balls)) {
            if (balls[i].position > balls[nBall].position) { largerPos.push(balls[i].position) }
            else { smallerPos.push(balls[i].position) }
        }
    }
    const positions = [...largerPos.sort(function (a, b) {
        return a - b;
...
export function getSwitchingMoves(balls: tBall.ballsType, nBall: number): number[] {
    ...
    for (let i = 0; i < balls.length; i++) {
        if (i === nBall) { continue; }
        if (balls[i].position < ballGoal(0, balls) && balls[i].position >= ballStart(0, balls)) { resultingPositions.push(balls[i].position) }
    }
...
export function getMovingPositions(balls: tBall.ballsType, nBall: number, cardTitle: tCard.cardType): number[] {
    ...
    for (let move = 0; move < cardTitleNumber; move++) {
        ...
        else { // in Goal also remove positions that are occupied
            endNodes = endNodes.filter(position => (position < ballGoal(0, balls)) || (!(balls.some(ball => ball.position === position))))
        }
        startNodes = [...endNodes]
    }
...
export function moveOneStep(balls: tBall.ballsType, nBall: number, ballPosition: number, direction: number, cardNumber: number): number[] {
    if (ballPosition < ballStart(0, balls)) { return [] }

    const resultingPositions = []
    if (direction === 1) { // FORWARD MOVEMENT
        if (ballPosition === ballStart(nBall, balls) // move into goal if allowed
            && balls[nBall].state === 'valid') {
            resultingPositions.push(ballGoal(nBall, balls))
        }

        // perform ordinary move
        if (ballPosition < ballGoal(0, balls)) { // not in goal
            if (ballPosition === ballGoal(0, balls) - 1) { resultingPositions.push(ballStart(0, balls)) } // close circle
            else { resultingPositions.push(ballPosition + 1) }
        }
        else { // in goal
            if (cardNumber === 7) {
            ...
            else { // move if not in end
                if (ballPosition < ballGoal(nBall, balls) + 3) { resultingPositions.push(ballPosition + 1) }
            }
        }
    }
    else { // BACKWARDS MOVEMENT - only possible with -4
        if (ballPosition === ballStart(nBall, balls) // move into goal if not first move
            && balls[nBall].state === 'valid') {
            resultingPositions.push(ballGoal(nBall, balls))
        }

        if (ballPosition >= ballGoal(0, balls)) { // move in house
            if (ballPosition < ballGoal(nBall, balls) + 3) { resultingPositions.push(ballPosition + 1) }
        }
        else { // move in circle
            if (ballPosition === ballStart(0, balls)) { resultingPositions.push(ballGoal(0, balls) - 1) } // close circle
            else { resultingPositions.push(ballPosition - 1) } // default
        }
    }
    return resultingPositions
}
