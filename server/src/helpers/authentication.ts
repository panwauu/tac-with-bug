...
export function generalSocketIOAuthentication(socket: GeneralSocketS, next: any) {
    if (socket.handshake.auth.token == null) { return next() }
...
export async function expressAuthentication(
    ...
    if (decoded.isErr()) { return Promise.reject({ msg: 'Your session is not valid!' }) }
    request.userData = { userID: decoded.value.userID }

    if (scopes === undefined || scopes.length === 0) { return Promise.resolve() }

    if (scopes.includes('admin')) {
        const admin = await isAdmin(request.app.locals.sqlClient, decoded.value.userID)
        if (admin.isOk() && admin.value) { return Promise.resolve() }
    }
...
async function isAdmin(pgPool: pg.Pool, userID: number): Promise<Result<boolean, 'USER_NOT_FOUND'>> {
    ...
    if (dbRes.rowCount === 0) { return err('USER_NOT_FOUND') }
