...
class PostgresTransport extends Transport {
    ...
    log(info: any, callback: any) {
        ...
        this.pgPool.query(`INSERT INTO ${this.tableName} (level, message, meta) VALUES ($1, $2, $3);`, [level, message, JSON.stringify(meta)])
            .catch((error) => {
...
function getLoggingDatabaseURL(): string | null {
    ...
    if (databaseEnvKey === undefined) { return null }
...
if (databaseUrl != null) {
    loggerOptions.transports.push(new PostgresTransport({
        level: 'warn',
...
if (process.env.NODE_ENV === 'production') { logger.warn('This is a test warning to test if the postgres logger is working when server starts', { reason: 'test' }) }
