...
describe('Test action normalization', () => {
    ...
    test('Un/normalize should not change for normalized game', () => {
        expect(normalizeAction([0, 0, 'abwerfen'], { cardsNewOrder: [0, 1, 2, 3, 4], ballsNewOrder: Array.from(Array(16).keys()), game: definedGame, playersShiftedBy: 0 })).toEqual([0, 0, 'abwerfen'])
        expect(unnormalizeAction([0, 0, 'abwerfen'], { cardsNewOrder: [0, 1, 2, 3, 4], ballsNewOrder: Array.from(Array(16).keys()), game: definedGame, playersShiftedBy: 0 })).toEqual([0, 0, 'abwerfen'])
    })
