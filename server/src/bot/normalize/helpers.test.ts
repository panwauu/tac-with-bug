...
describe('Test helper functions', () => {
    ...
    test('Array should be ordered for random array', () => {
        const order = Array.from(Array(10).keys()).sort(() => { return Math.random() - 0.5 })
    ...
    test('Order Array should be reversible for random array and random order', () => {
        const array = Array(10).fill(0).map(() => Math.random())
        const order = Array.from(Array(array.length).keys()).sort(() => { return Math.random() - 0.5 })
