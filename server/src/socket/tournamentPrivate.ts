...
export async function registerTournamentPrivateHandler(pgPool: pg.Pool, socket: GeneralSocketS) {
    ...
    socket.on('tournament:private:planAddPlayer', async (data, cb) => {
        ...
        try {
            ...
            sendPrivateTournamentInvitation({ user: invitedUser.value, tournamentTitle: newTournament.value.title, invitingPlayer: newTournament.value.adminPlayer, teamName: data.teamTitle })
    ...
    socket.on('tournament:private:startGame', async (data, cb) => {
        ...
        try {
            ...
            if (!(gamePlayers.includes(socket.data.userID) || tournament.adminPlayerID === socket.data.userID)) { return cb({ status: 500, error: 'ONLY_ADMIN_PLAYER_OR_PLAYER_OF_GAME' }) }
...
export function pushChangedPrivateTournament(tournament: tTournament.privateTournament) { nspGeneral.emit('tournament:private:update', tournament) }
