...
async function onGet(pgPool: pg.Pool, socket: GeneralSocketS) {
    if (socket.data.userID === undefined) { emitSubscription(socket, getInvalidSubscriptionDetails()); return }

    const sub = await getSubscription(pgPool, socket.data.userID)
    if (sub.isErr()) { logger.error(sub.error); return }
    emitSubscription(socket, sub.value)
}

export async function registerSubscriptionHandler(pgPool: pg.Pool, socket: GeneralSocketS) {
    socket.on('subscription:get', () => { onGet(pgPool, socket) });

    socket.on('subscription:new', async (subscriptionID, callback) => {
        if (socket.data.userID === undefined) { logger.error('Event forbidden for unauthenticated user (subscription:new)', { stack: new Error().stack }); return }
        ...
        if (error?.details[0]?.path[0] === 'callback') { return }
        ...
        try {
            ...
            if (newSubRes.isErr()) { throw new Error(newSubRes.error) }
            onGet(pgPool, socket)
    ...
    socket.on('subscription:cancel', async (callback) => {
        if (socket.data.userID === undefined) { logger.error('Event forbidden for unauthenticated user (subscription:cancel)', { stack: new Error().stack }); return }
    ...
    socket.on('subscription:nSubscriptions', async () => {
        ...
        } catch (err) { logger.error('Subscription Handler Socket', err) }
    });
}