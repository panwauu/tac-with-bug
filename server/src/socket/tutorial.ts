...
export async function initializeTutorial(pgPool: pg.Pool, socket: GeneralSocketS) {
    ...
    if (socket.data.userID != null) {
        ...
        if (userProgress.isOk()) { progess = userProgress.value }
    }
...
export async function registerTutorialHandler(pgPool: pg.Pool, socket: GeneralSocketS) {
    socket.on('tutorial:load', async (data, callback) => {
        ...
        const dbRes = await pgPool.query<{ size: number, tutorial_step_definition: TutorialStepDefinition }>('SELECT jsonb_array_length(data) as size, data->CAST($2 AS INT) as tutorial_step_definition FROM tutorials WHERE id=$1;', [data.tutorialID, data.tutorialStep])
        if (dbRes.rowCount === 0 || data.tutorialStep + 1 > dbRes.rows[0].size) { return callback({ status: 500, error: 'Could not query Tutorial Step' }) }
    ...
    socket.on('tutorial:changeTutorialStep', async (data, callback) => {
        if (socket.data.userID === undefined) { logger.error('Event forbidden for unauthenticated user (tutorial:changeTutorialStep)', { stack: new Error().stack }); return callback({ status: 500 }) }
    ...
    socket.on('tutorial:resetTutorial', async (data, callback) => {
        if (socket.data.userID === undefined) { logger.error('Event forbidden for unauthenticated user (tutorial:resetTutorial)', { stack: new Error().stack }); return callback({ status: 500 }) }
