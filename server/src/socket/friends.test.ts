...
describe('Friends test suite via socket.io', () => {
  ...
  test('Player should not be able to befriend himself', async () => {
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[0].socket.emit('friends:request', usersWithSockets[0].username, (data) => {
        resolve(data)
      })
    )
    expect(response.status).toBe(500)
  ...
  test('Player should not be able to befriend invalid player', async () => {
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[0].socket.emit('friends:request', 'a', (data) => {
        resolve(data)
      })
    )
    expect(response.status).toBe(500)
  ...
  test('Should be able to request friendship from other player and alert player', async () => {
    const promises: [Promise<friend[]>, Promise<friend[]>, Promise<{ username: string }>] = [
      ...
      new Promise((resolve) =>
        usersWithSockets[1].socket.once('friends:update', (data) => {
          resolve(data)
        })
      ),
      new Promise((resolve) =>
        usersWithSockets[1].socket.once('friends:new-request', (data) => {
          resolve(data)
        })
      ),
    ]
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[0].socket.emit('friends:request', usersWithSockets[1].username, (data) => {
        resolve(data)
      })
    )
  ...
  test('Player should not be able to confirm a friendship which was initiated by himself', async () => {
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[0].socket.emit('friends:confirm', usersWithSockets[1].username, (data) => {
        resolve(data)
      })
    )
    expect(response.status).toBe(500)
  ...
  test('Player should not be able to confirm a friendship where there is none', async () => {
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[2].socket.emit('friends:confirm', usersWithSockets[1].username, (data) => {
        resolve(data)
      })
    )
    expect(response.status).toBe(500)
  ...
  test('Should be able to confirm friendship', async () => {
    const promises: [Promise<friend[]>, Promise<friend[]>, Promise<{ username: string }>] = [
      ...
      new Promise((resolve) =>
        usersWithSockets[0].socket.once('friends:friend-confirmed', (data) => {
          resolve(data)
        })
      ),
    ]
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[1].socket.emit('friends:confirm', usersWithSockets[0].username, (data) => {
        resolve(data)
      })
    )
    const result = await Promise.all(promises)
  ...
  test('Uninvolved player should not be able to cancel unexisting friendship', async () => {
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[2].socket.emit('friends:cancel', usersWithSockets[0].username, (data) => {
        resolve(data)
      })
    )
    expect(response.status).toBe(500)
  ...
  test('Should be able to cancel friendship', async () => {
    ...
    const promises: [Promise<friend[]>, Promise<friend[]>, Promise<{ username: string }>] = [
      ...
      new Promise((resolve) =>
        usersWithSockets[playerToBeCanceled].socket.once('friends:friend-cancelled', (data) => {
          resolve(data)
        })
      ),
    ]
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[playerToCancel].socket.emit('friends:cancel', usersWithSockets[playerToBeCanceled].username, (data) => {
        resolve(data)
      })
    )
    const result = await Promise.all(promises)
  ...
  test('User should be able to withdraw request', async () => {
    ...
    const responseRequest = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[0].socket.emit('friends:request', usersWithSockets[1].username, (data) => {
        resolve(data)
      })
    )
    ...
    const promises: [Promise<friend[]>, Promise<friend[]>, Promise<{ username: string }>] = [
      ...
      new Promise((resolve) =>
        usersWithSockets[1].socket.once('friends:friend-withdrew', (data) => {
          resolve(data)
        })
      ),
    ]
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[0].socket.emit('friends:cancel', usersWithSockets[1].username, (data) => {
        resolve(data)
      })
    )
    const result = await Promise.all(promises)
  ...
  test('User should be able to decline request', async () => {
    ...
    const promises: [Promise<friend[]>, Promise<friend[]>, Promise<{ username: string }>] = [
      ...
      new Promise((resolve) =>
        usersWithSockets[0].socket.once('friends:friend-declined', (data) => {
          resolve(data)
        })
      ),
    ]
    const response = await new Promise<AckData<boolean>>((resolve) =>
      usersWithSockets[1].socket.emit('friends:cancel', usersWithSockets[0].username, (data) => {
        resolve(data)
      })
    )
    const result = await Promise.all(promises)
